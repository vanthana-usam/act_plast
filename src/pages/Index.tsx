...
const Index = () => {
  ...
  const { taskCount, refreshTaskCount, isLoading, error } = useTaskContext();
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      ...
      try {
        ...
        setAllowedTabs(data[user.employeeGroup] || []);
      } catch (err) {
  ...
  const allMenuItems = [
    ...
    { id: "preventivemaintenance", title: "PreventiveMaintenance", icon: ClipboardList },
    { id: "tabs", title: "Tab Permissions", icon: Settings },
  ];

  const isAdmin = user?.employeeGroup?.toLowerCase() === "admin";
  const allowedMenuItems = isAdmin ? allMenuItems : allMenuItems.filter((item) => allowedTabs.includes(item.id));

  const renderContent = () => {
    const activeComponent = allowedMenuItems.find((item) => item.id === activeView)?.id || "dashboard";
            ...
            <div className="flex items-center justify-between px-6 py-4">
              <div className="flex items-center space-x-4">
                ...
                <h2 className="text-xl font-semibold text-gray-900 capitalize">
                  {allowedMenuItems.find((item) => item.id === activeView)?.title || "Dashboard"}
                </h2>
              </div>
              <div className="flex items-center space-x-4">
...
// // ... (rest of the Index component remains the same)
...
//         });
//         const data = await res.json();
//         const allowedTabs = data[user.employeeGroup] || [];
//         const isAdmin = user.employeeGroup?.toLowerCase() === "admin";
//         const filtered = isAdmin ? allMenuItems : allMenuItems.filter((item) => allowedTabs.includes(item.id));
