...
const ReportsSection = () => {
  ...
  useEffect(() => {
    const fetchMOM = async () => {
      try {
        ...
        if (!response.ok) throw new Error('Failed to fetch MOM data');
        const data = await response.json();
        setMomRecords(data);
      } catch (error) {
  ...
  const fetchPMRecords = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error('Failed to fetch preventive maintenance records');
      }

      const result = await response.json();
      if (result.success) {
        setPmRecords(result.data);
        refreshTaskCount(); // Update task count if needed
      } else {
  ...
  // OEE, Downtime, Rejection, and Trend calculations (unchanged)
  useEffect(() => {
  ...
  // Helper functions for PM metrics
  const isCompletedThisMonth = (record) => {
    if (!record.CompletedAt || record.Status !== 'completed') return false;
  ...
  const isOverdue = (record) => {
    if (!record.DueDate || record.Status === 'completed') return false;
    const dueDate = new Date(record.DueDate);
    const now = new Date();
    return dueDate < now;
  };

  const completedThisMonth = pmRecords.filter(isCompletedThisMonth).length;
  const pendingMaintenance = pmRecords.filter(rec => rec.Status == 'pending').length;
  const progressMaintenance = pmRecords.filter(rec => rec.Status == 'in progress').length;
  const overdueChecks = pmRecords.filter(isOverdue).length;
  ...
  const exportPMData = () => {
    const headers = ['Id', 'EquipmentType', 'DueDate', 'Description', 'Status', 'CreatedAt', 'UpdatedAt', 'CompletedAt'];
    exportToCSV(pmRecords, 'pm_report.csv', headers);
  };
  ...
  const exportConsolidatedData = () => {
    ...
    // PM Sheet
    const pmSheetData = pmRecords.map(item => ({
      Id: item.Id,
  ...
  // Rest of the export functions (OEE, Downtime, Rejection, Trend, MOM) remain unchanged
  const exportOEEData = () => {
          ...
          <Card className="border-0 shadow-md">
            ...
            <CardContent>
              {pmLoading ? (
                ...
                <div className="text-red-500">Error: {pmError}</div>
              ) : (
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                        ...
                        <Badge className="bg-yellow-500">{pendingMaintenance}</Badge>
                      </div>
                      <div className="flex justify-between items-center p-3 bg-orange-50 rounded-lg">
                        <span>In Progress Maintenance</span>
                        <Badge className="bg-green-500">{progressMaintenance}</Badge>
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
                        ...
                        <Badge className="bg-red-500">{overdueChecks}</Badge>
                      </div>
                    </div>
                  </div>
                </div>
