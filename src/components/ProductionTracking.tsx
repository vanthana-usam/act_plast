...
const rejectionTypes = ["Startup", "In Process", "Re Startup"];
...
const ProductionTracking: React.FC = React.memo(() => {
  ...
  const [formErrors, setFormErrors] = useState<
    Partial<
      Record<keyof ProductionFormData, string> & {
        ...
        downtimeEntries?: { [index: number]: Partial<DowntimeEntry> };
  ...
  const filteredProduction = useMemo(() => {
    ...
    if (filterPhenomenon !== "all") {
      filtered = filtered.filter((record) =>
        record.downtimeEntries.some((entry) => entry.type === filterPhenomenon)
      );
    }
  ...
  // Handle downtime entry changes
  const handleDowntimeEntryChange = useCallback(
    (index: number, field: keyof DowntimeEntry, value: any) => {
      setFormData((prev) => ({
        ...prev,
        downtimeEntries: prev.downtimeEntries.map((entry, i) =>
          i === index ? { ...entry, [field]: value } : entry
        ),
      }));
    },
  ...
  const addDowntimeEntry = useCallback(() => {
    setFormData((prev) => ({
      ...prev,
      downtimeEntries: [
        ...prev.downtimeEntries,
        {
          type: "",
          downtimeMinutes: "",
          reason: "",
  ...
  const removeDowntimeEntry = useCallback(
    (index: number) => {
      setFormData((prev) => {
        const newDowntimeEntries = prev.downtimeEntries.filter((_, i) => i !== index);
        const newErrors = { ...formErrors };
        if (newErrors.downtimeEntries?.[index]) {
          delete newErrors.downtimeEntries[index];
          if (!Object.keys(newErrors.downtimeEntries).length) {
            delete newErrors.downtimeEntries;
          }
        }
        setFormErrors(Object.keys(newErrors).length ? newErrors : {});
        return { ...prev, downtimeEntries: newDowntimeEntries };
      });
    },
    [formErrors]
  );
  ...
  const validateForm = useCallback(() => {
    ...
    const downtimeErrors: { [index: number]: Partial<DowntimeEntry> } = {};
    const nonEmptyDowntimes = formData.downtimeEntries.filter(
      (entry) => entry.type || entry.downtimeMinutes || entry.reason || entry.customType
    );

    nonEmptyDowntimes.forEach((entry, index) => {
      const entryErrors: Partial<DowntimeEntry> = {};
      if (!entry.type) entryErrors.type = "Downtime type is required.";
      if (entry.type === "other" && !entry.customType) {
        entryErrors.customType = "Custom downtime type is required.";
      }
      if (!entry.downtimeMinutes) {
        entryErrors.downtimeMinutes = "Duration is required.";
      } else if (isNaN(Number(entry.downtimeMinutes)) || Number(entry.downtimeMinutes) < 0) {
        entryErrors.downtimeMinutes = "Must be a non-negative number.";
      }
      if (entry.reason && !isNaN(Number(entry.reason))) {
        entryErrors.reason = "Reason cannot be numeric.";
      }
      if (entry.correctiveActions?.length) {
        const actionErrors: { [actionIndex: number]: Partial<Action> } = {};
      ...
      if (Object.keys(entryErrors).length) downtimeErrors[index] = entryErrors;
    });

    if (Object.keys(downtimeErrors).length) {
      errors.downtimeEntries = downtimeErrors;
    }

    const totalDowntime = nonEmptyDowntimes.reduce(
      (sum, entry) => sum + Number(entry.downtimeMinutes) || 0,
      0
    );
    if (formData.plannedMins && totalDowntime > Number(formData.plannedMins)) {
      errors.downtimeEntries = {
        ...errors.downtimeEntries,
        general: "Total downtime cannot exceed planned minutes.",
      };
    }
  ...
  const handleSubmitProduction = useCallback(async () => {
    ...
    const rejectionTypesFinal = formData.rejectionEntries
      .filter((entry) => entry.type || entry.quantity || entry.reason || entry.customType || entry.assignToTeam)
      .map((entry) => ({
        entryId: generateUUID(),
        type: entry.type === "Other" ? entry.customType || "Other" : entry.type,
        quantity: Number(entry.quantity) || 0,
        reason: entry.reason || "",
        assignToTeam: entry.assignToTeam || "",
        correctiveActions: entry.correctiveActions?.filter((action) => action.action) || [],
      }));

    const downtimeEntriesFinal = formData.downtimeEntries
      .filter((entry) => entry.type || entry.downtimeMinutes || entry.reason || entry.customType || entry.assignToTeam)
      .map((entry) => ({
        entryId: generateUUID(),
        type: entry.type === "other" ? entry.customType || "other" : entry.type,
    ...
    const newRecord: ProductionRecord = {
      ...
      downtime: downtimeEntriesFinal.reduce((sum, entry) => sum + entry.downtimeMinutes, 0),
      downtimeEntries: downtimeEntriesFinal,
    ...
    try {
      ...
      setFormData({
        ...formInitialState,
        rejectionEntries: [{ type: "", quantity: "", reason: "", customType: "", assignToTeam: "", correctiveActions: [] }],
        downtimeEntries: [{ type: "", downtimeMinutes: "", reason: "", customType: "", assignToTeam: "", correctiveActions: [] }],
      });
      setFormErrors({});
      setCorrectiveActions([{ id: generateUUID(), action: ""}]);
  ...
  const exportToCSV = useCallback(() => {
    if (filteredProduction.length === 0) {
      toast({ variant: "destructive", title: "No Data", description: "No production data to export." });
      return;
    }
    ...
    const headers = [
      'Production Code', 'Production Type', 'Date', 'Shift', 
      'Machine Name', 'Product', 'Planned Qty', 'Actual Qty', 'Rejected Qty', 
      'Lumps Qty', 'Lumps Reason', 'Downtime (min)', 'Defect Type', 
      'Target Output', 'Planned Mins', 'Operator', 'Supervisor', 'Status', 
      'Efficiency (%)', 'Team', 'Custom Defect Type'
    ];

    const csvRows = filteredProduction.map((record) => [
      // record.recordId,
  ...
  }, [filteredProduction, getEmployeeName, toast]);
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="space-x-2">
          <Button
            onClick={exportToCSV}
            variant="outline"
            aria-label="Export production data to CSV"
          >
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white">
              ...
              >
                <div className="space-y-6 py-4">
                    ...
                    {formData.rejectionEntries.map((entry, index) => (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          {renderSelect({
                            id: `assignToTeam-${index}`,
                            value: entry.assignToTeam,
                            onChange: (value) =>
                              handleRejectionEntryChange(
                                index,
                                "assignToTeam",
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <Label className="font-medium">Downtime Entries</Label>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={addDowntimeEntry}
                        aria-label="Add new downtime entry"
                      >
                        <Plus className="h-4 w-4 mr-2" /> Add Downtime
                      </Button>
                    </div>
                    {formErrors.downtimeEntries?.general && (
                      <p className="text-red-500 text-sm mb-2">
                        {formErrors.downtimeEntries.general}
                      </p>
                    )}
                    {formData.downtimeEntries.map((entry, index) => (
                      <div
                        ...
                        className="border p-4 rounded-lg mb-4 relative bg-gray-50"
                      >
                        {formData.downtimeEntries.length > 1 && (
                          <Button
                            variant="ghost"
                            size="icon"
                            className="absolute top-2 right-2"
                            onClick={() => removeDowntimeEntry(index)}
                            aria-label={`Remove downtime entry ${index + 1}`}
                          >
                            <Trash2 className="h-4 w-4 text-red-500" />
                          </Button>
                        )}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {renderSelect({
                            id: `downtimeType-${index}`,
                            value: entry.type,
                            onChange: (value) =>
                              handleDowntimeEntryChange(index, "type", value),
                            options: downtimeTypes.map((type) => ({
                              value: type.value,
                              label: type.label,
                            })),
                            placeholder: "Select downtime type",
                            error: formErrors.downtimeEntries?.[index]?.type,
                            label: "Downtime Type",
                          ...
                          {entry.type === "other" && (
                            <div className="flex flex-col space-y-1.5">
                              <Label
                                htmlFor={`customDowntimeType-${index}`}
                                className="font-medium"
                              >
                                Custom Type
                              </Label>
                              <Input
                                id={`customDowntimeType-${index}`}
                                placeholder="Specify custom type"
                                value={entry.customType}
                                onChange={(e) =>
                                  handleDowntimeEntryChange(
                                    index,
                                    "customType",
                                    e.target.value
                                  )
                                ...
                                className={`w-full ${
                                  formErrors.downtimeEntries?.[index]?.customType
                                    ? "border-red-500"
                                    : "border-gray-300"
                                }`}
                                aria-invalid={
                                ...
                                aria-describedby={
                                  formErrors.downtimeEntries?.[index]?.customType
                                    ? `customDowntimeType-error-${index}`
                                    : undefined
                                }
                              />
                              {formErrors.downtimeEntries?.[index]?.customType && (
                                <p
                                  id={`customDowntimeType-error-${index}`}
                                  className="text-red-500 text-sm"
                                >
                                  {formErrors.downtimeEntries[index].customType}
                                </p>
                          ...
                          <div className="flex flex-col space-y-1.5">
                            <Label
                              htmlFor={`downtimeDuration-${index}`}
                              className="font-medium"
                            >
                              Duration (minutes)
                            </Label>
                            <Input
                              id={`downtimeDuration-${index}`}
                              type="number"
                              placeholder="30"
                              ...
                              className={`w-full ${
                                formErrors.downtimeEntries?.[index]?.downtimeMinutes
                                  ? "border-red-500"
                                  : "border-gray-300"
                              }`}
                              aria-invalid={
                              ...
                              aria-describedby={
                                formErrors.downtimeEntries?.[index]?.downtimeMinutes
                                  ? `downtimeDuration-error-${index}`
                                  : undefined
                              }
                            />
                            {formErrors.downtimeEntries?.[index]?.downtimeMinutes && (
                              <p
                                id={`downtimeDuration-error-${index}`}
                                className="text-red-500 text-sm"
                              >
                                {formErrors.downtimeEntries[index].downtimeMinutes}
                              </p>
                          ...
                          {renderSelect({
                            ...
                            isLoading: isLoading.employees,
                            error:
                              formErrors.downtimeEntries?.[index]
                                ?.assignToTeam,
                            label: "Assign to Team",
                        ...
                        <div className="mt-4">
                          ...
                          <Textarea
                            ...
                            className={`w-full ${
                              formErrors.downtimeEntries?.[index]?.reason
                                ? "border-red-500"
                                : "border-gray-300"
                            }`}
                            aria-invalid={
                            ...
                            aria-describedby={
                              formErrors.downtimeEntries?.[index]?.reason
                                ? `downtimeReason-error-${index}`
                                : undefined
                            }
                          />
                          {formErrors.downtimeEntries?.[index]?.reason && (
                            <p
                              id={`downtimeReason-error-${index}`}
                              className="text-red-500 text-sm"
                            >
                              {formErrors.downtimeEntries[index].reason}
                            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            {renderSelect({
              ...
              options: [
                ...
                { value: "in-proceess", label: "In Process" },
              ...
              {filteredProduction.map((record) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-4">
                    ...
                    <div className="bg-gray-50 p-3 rounded-lg">
                      ...
                      <p className="text-lg font-bold text-gray-900">
                        {record.downtimeEntries?.reduce((total, entry) => total + (entry.downtimeMinutes || 0), 0)} min
                      </p>
                  ...
                  {(record.rejectionTypes?.some((entry) => entry.reason) ||
                    record.lumpsReason ||
                    record.downtimeEntries?.some((entry) => entry.reason)) && (
                    <div className="bg-yellow-50 p-4 rounded-lg">
                      <div className="flex items-start space-x-2">
                        ...
                        <div>
                          ...
                          {record.downtimeEntries?.map(
                            (entry, index) =>
                              entry.reason && (
                                <div key={index} className="mb-2">
                                  <p className="text-sm font-medium text-yellow-800">
                                    Downtime ({entry.type}): {entry.downtimeMinutes} min
                                  </p>
                                  <p className="text-sm text-yellow-700">
                                    {entry.reason}
                                  </p>
